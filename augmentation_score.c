...
void augmentation_score(char pseudo[], int augmentation) {
    /// Cette fonction augmente de augmentation le score dans scores.txt à la ligne correspondant à celle de pseudo dans pseudos.txt
    ...
    // On augmente de augmentation le score dans scores.txt correspondant à la ligne dans pseudos.txt
    scores_lus[numero_de_la_ligne_du_pseudo] = scores_lus[numero_de_la_ligne_du_pseudo] + augmentation;

    // On ré-écrit les scores
    FILE* pFichierScoresW = fopen("./donnees_sauvegardees/scores.txt", "w"); // Ouverture du fichier scores.txt en mode écriture
    if (pFichierScoresW == NULL) {
        printf("Erreur d'ouverture de fichier");
        exit(1);
    }
    for (int j = 0; j <= nl_scores; j++) {fprintf(pFichierScoresW, "%d", scores_lus[j]);fprintf(pFichierScoresW, "%s", "\n");}
    fclose(pFichierScoresW);
    pFichierScoresW = NULL;
}